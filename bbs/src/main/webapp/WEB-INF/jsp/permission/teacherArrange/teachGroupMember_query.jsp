			...
			<div class="researchDiv">
				...
				<div class="selectResource" id="PrintArea">
					<table width="100%" cellpadding="0" cellspacing="0" >
						...
						<c:forEach items="${teachGroupMemberList}" var="teachMemberVO">
						<tr>
							<td class="fontBold" rowspan="2">${teachMemberVO.teachGroup.name}</td>
							<td>组长</td>
							<td>
								<div>
									<c:forEach items="${teachMemberVO.teachGroupLeader}" var="user">
						...
						<tr>
							...
							<td>
								<div>
									<c:forEach items="${teachMemberVO.teachGroupMember}" var="user">
										<input type="hidden" name="userId" value="${user.userId}" />
        									${user.name}
        							</c:forEach>
								</div>
