...
@Controller("add${table.className}Action")
@Scope("prototype")
public class Add${table.className}Action extends BaseAction {
	...
	public String execute() throws Exception {
		try {
			${table.lowerCaseFirstClassName}Service.save(${table.lowerCaseFirstClassName}Dto);
			return SUCCESS;
		} catch (Exception ex) {
			ex.printStackTrace();
		}
