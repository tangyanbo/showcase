...
<body class="easyui-layout">
			...
			<div class="easyui-panel" style="padding:0px;margin: 0px;" data-options="region:'center'">
				<table  id="dg_agentModel" class="easyui-datagrid" 
...
jQuery(function($){
	//定义构造查询
	agentModel_list.buildQueryParams=function(){
	    $('#dg_agentModel').datagrid({
			queryParams: {
	...
	$('#search_btn').click(function(){
		agentModel_list.buildQueryParams();
		$('#dg_agentModel').datagrid('load');
	});
	...
	$('#add_btn').click(function(){
		var add_form_id ='#addAgentModelFrom';
		$('<div id="dialog_holder"></div>').dialog({
		    title: 'AGENT_MODEL',
		    width: 800,
		    height: 500,
		    href: 'addAgentModelPage.action',
		    modal: true,
		    method: "POST",
		    ...
		    buttons: [{
		    	text: "提  交",
		    	handler: function(e){
		    		$('#addAgentModelFrom').form({   
						 url:'addAgentModelAction.action',    
							     onSubmit: function(){
							       $.messager.progress(); 
							        // do some check       
	...
	//更新
	agentModel_list.updateFormSubmit = function(pkid){
	    var edit_form_id ='#editAgentModelFrom';
		$('<div id="dialog_holder"></div>').dialog({
		    title: 'AgentModel',
		    width: 800,
		    height: 500,
		    href: 'editAgentModelPage.action',
		    modal: true,
		    method: "POST",
		    params:{pkid:pkid},
		    extractor: function(data){
		    ...
		    buttons: [{
		    	text: "保  存",
		    	handler: function(e){
		   				$('#editAgentModelFrom').form({   
						 url:'editAgentModelAction.action',    
							     onSubmit: function(){
							       $.messager.progress(); 
							        // do some check       
							        // return false to prevent submit;    
		    ...
		    ,{
		    	text: "删  除",
		     	handler: function(e){
		     			$('#delAgentModelFrom').form({   
						 url:'delAgentModelAction.action',    
							     onSubmit: function(){
							       $.messager.progress(); 
							        // do some check       
							        // return false to prevent submit;    
	...
	//详细
	agentModel_list.detailFormSubmit = function(pkid){
		$('<div id="dialog_holder"></div>').dialog({
		    title: '详细AgentModel',
		    width: 800,
		    height: 500,
		    href: 'detailAgentModelPage.action',
		    modal: true,
		    method: "POST",
		    params:{pkid:pkid},
		    extractor: function(data){
