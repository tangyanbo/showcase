...
<body class="easyui-layout">
			...
			<div class="easyui-panel" style="padding:0px;margin: 0px;" data-options="region:'center'">
				<table  id="dg_platModel" class="easyui-datagrid" 
...
jQuery(function($){
	//定义构造查询
	platModel_list.buildQueryParams=function(){
	    $('#dg_platModel').datagrid({
			queryParams: {
	...
	$('#search_btn').click(function(){
		platModel_list.buildQueryParams();
		$('#dg_platModel').datagrid('load');
	});
	...
	$('#add_btn').click(function(){
		var add_form_id ='#addPlatModelFrom';
		$('<div id="dialog_holder"></div>').dialog({
		    title: 'PLAT_MODEL',
		    width: 800,
		    height: 500,
		    href: 'addPlatModelPage.action',
		    modal: true,
		    method: "POST",
		    ...
		    buttons: [{
		    	text: "提  交",
		    	handler: function(e){
		    		$('#addPlatModelFrom').form({   
						 url:'addPlatModelAction.action',    
							     onSubmit: function(){
							       $.messager.progress(); 
							        // do some check       
	...
	//更新
	platModel_list.updateFormSubmit = function(pkid){
	    var edit_form_id ='#editPlatModelFrom';
		$('<div id="dialog_holder"></div>').dialog({
		    title: 'PlatModel',
		    width: 800,
		    height: 500,
		    href: 'editPlatModelPage.action',
		    modal: true,
		    method: "POST",
		    params:{pkid:pkid},
		    extractor: function(data){
		    ...
		    buttons: [{
		    	text: "保  存",
		    	handler: function(e){
		   				$('#editPlatModelFrom').form({   
						 url:'editPlatModelAction.action',    
							     onSubmit: function(){
							       $.messager.progress(); 
							        // do some check       
							        // return false to prevent submit;    
		    ...
		    ,{
		    	text: "删  除",
		     	handler: function(e){
		     			$('#delPlatModelFrom').form({   
						 url:'delPlatModelAction.action',    
							     onSubmit: function(){
							       $.messager.progress(); 
							        // do some check       
							        // return false to prevent submit;    
	...
	//详细
	platModel_list.detailFormSubmit = function(pkid){
		$('<div id="dialog_holder"></div>').dialog({
		    title: '详细PlatModel',
		    width: 800,
		    height: 500,
		    href: 'detailPlatModelPage.action',
		    modal: true,
		    method: "POST",
		    params:{pkid:pkid},
		    extractor: function(data){
