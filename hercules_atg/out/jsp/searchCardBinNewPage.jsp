...
<body class="easyui-layout">
			...
			<div class="easyui-panel" style="padding:0px;margin: 0px;" data-options="region:'center'">
				<table  id="dg_cardBinNew" class="easyui-datagrid" 
...
jQuery(function($){
	//定义构造查询
	cardBinNew_list.buildQueryParams=function(){
	    $('#dg_cardBinNew').datagrid({
			queryParams: {
	...
	$('#search_btn').click(function(){
		cardBinNew_list.buildQueryParams();
		$('#dg_cardBinNew').datagrid('load');
	});
	...
	$('#add_btn').click(function(){
		var add_form_id ='#addCardBinNewFrom';
		$('<div id="dialog_holder"></div>').dialog({
		    title: 'CARD_BIN_NEW',
		    width: 800,
		    height: 500,
		    href: 'addCardBinNewPage.action',
		    modal: true,
		    method: "POST",
		    ...
		    buttons: [{
		    	text: "提  交",
		    	handler: function(e){
		    		$('#addCardBinNewFrom').form({   
						 url:'addCardBinNewAction.action',    
							     onSubmit: function(){
							       $.messager.progress(); 
							        // do some check       
	...
	//更新
	cardBinNew_list.updateFormSubmit = function(pkid){
	    var edit_form_id ='#editCardBinNewFrom';
		$('<div id="dialog_holder"></div>').dialog({
		    title: 'CardBinNew',
		    width: 800,
		    height: 500,
		    href: 'editCardBinNewPage.action',
		    modal: true,
		    method: "POST",
		    params:{pkid:pkid},
		    extractor: function(data){
		    ...
		    buttons: [{
		    	text: "保  存",
		    	handler: function(e){
		   				$('#editCardBinNewFrom').form({   
						 url:'editCardBinNewAction.action',    
							     onSubmit: function(){
							       $.messager.progress(); 
							        // do some check       
							        // return false to prevent submit;    
		    ...
		    ,{
		    	text: "删  除",
		     	handler: function(e){
		     			$('#delCardBinNewFrom').form({   
						 url:'delCardBinNewAction.action',    
							     onSubmit: function(){
							       $.messager.progress(); 
							        // do some check       
							        // return false to prevent submit;    
	...
	//详细
	cardBinNew_list.detailFormSubmit = function(pkid){
		$('<div id="dialog_holder"></div>').dialog({
		    title: '详细CardBinNew',
		    width: 800,
		    height: 500,
		    href: 'detailCardBinNewPage.action',
		    modal: true,
		    method: "POST",
		    params:{pkid:pkid},
		    extractor: function(data){
