...
<body class="easyui-layout">
			...
			<div class="easyui-panel" style="padding:0px;margin: 0px;" data-options="region:'center'">
				<table  id="dg_cardBin1103" class="easyui-datagrid" 
...
jQuery(function($){
	//定义构造查询
	cardBin1103_list.buildQueryParams=function(){
	    $('#dg_cardBin1103').datagrid({
			queryParams: {
	...
	$('#search_btn').click(function(){
		cardBin1103_list.buildQueryParams();
		$('#dg_cardBin1103').datagrid('load');
	});
	...
	$('#add_btn').click(function(){
		var add_form_id ='#addCardBin1103From';
		$('<div id="dialog_holder"></div>').dialog({
		    title: 'CARD_BIN1103',
		    width: 800,
		    height: 500,
		    href: 'addCardBin1103Page.action',
		    modal: true,
		    method: "POST",
		    ...
		    buttons: [{
		    	text: "提  交",
		    	handler: function(e){
		    		$('#addCardBin1103From').form({   
						 url:'addCardBin1103Action.action',    
							     onSubmit: function(){
							       $.messager.progress(); 
							        // do some check       
	...
	//更新
	cardBin1103_list.updateFormSubmit = function(pkid){
	    var edit_form_id ='#editCardBin1103From';
		$('<div id="dialog_holder"></div>').dialog({
		    title: 'CardBin1103',
		    width: 800,
		    height: 500,
		    href: 'editCardBin1103Page.action',
		    modal: true,
		    method: "POST",
		    params:{pkid:pkid},
		    extractor: function(data){
		    ...
		    buttons: [{
		    	text: "保  存",
		    	handler: function(e){
		   				$('#editCardBin1103From').form({   
						 url:'editCardBin1103Action.action',    
							     onSubmit: function(){
							       $.messager.progress(); 
							        // do some check       
							        // return false to prevent submit;    
		    ...
		    ,{
		    	text: "删  除",
		     	handler: function(e){
		     			$('#delCardBin1103From').form({   
						 url:'delCardBin1103Action.action',    
							     onSubmit: function(){
							       $.messager.progress(); 
							        // do some check       
							        // return false to prevent submit;    
	...
	//详细
	cardBin1103_list.detailFormSubmit = function(pkid){
		$('<div id="dialog_holder"></div>').dialog({
		    title: '详细CardBin1103',
		    width: 800,
		    height: 500,
		    href: 'detailCardBin1103Page.action',
		    modal: true,
		    method: "POST",
		    params:{pkid:pkid},
		    extractor: function(data){
