...
<body class="easyui-layout">
			...
			<div class="easyui-panel" title="查询条件" style="padding:5px;height:120px;margin: 0px;" data-options="region:'north'">
				<form id="searchForm" name="searchForm" method="post" >
					<table cellpadding="5">
								...
								<td>remark1</td>
								<td>
									<input type="text" id="search_remark1" name="search_remark1" class="easyui-textbox" value="${search_remark1}"/>
								</td>
					</table>
				</form>
			</div>
			...
			<div class="easyui-panel" style="padding:0px;margin: 0px;" data-options="region:'center'">
				<table  id="dg_regionCode" class="easyui-datagrid" 
					...
					<thead>
						<tr>
							...
							<th data-options="field:'REMARK1',width:120,sortable:'true',align:'left'">remark1</th>
						</tr>
					</thead>
				</table>
			</div>
...
jQuery(function($){
	//定义构造查询
	regionCode_list.buildQueryParams=function(){
	    $('#dg_regionCode').datagrid({
			queryParams: {
						 ...
						 'search_remark1':$("input[name='search_remark1']").val(),
			}
		});
	}
	...
	$('#search_btn').click(function(){
		regionCode_list.buildQueryParams();
		$('#dg_regionCode').datagrid('load');
	});
	...
	$('#add_btn').click(function(){
		var add_form_id ='#addRegionCodeFrom';
		$('<div id="dialog_holder"></div>').dialog({
		    title: 'REGION_CODE',
		    width: 800,
		    height: 500,
		    href: 'addRegionCodePage.action',
		    modal: true,
		    method: "POST",
		    ...
		    buttons: [{
		    	text: "提  交",
		    	handler: function(e){
		    		$('#addRegionCodeFrom').form({   
						 url:'addRegionCodeAction.action',    
							     onSubmit: function(){
							       $.messager.progress(); 
							        // do some check       
	...
	//更新
	regionCode_list.updateFormSubmit = function(pkid){
	    var edit_form_id ='#editRegionCodeFrom';
		$('<div id="dialog_holder"></div>').dialog({
		    title: 'RegionCode',
		    width: 800,
		    height: 500,
		    href: 'editRegionCodePage.action',
		    modal: true,
		    method: "POST",
		    params:{pkid:pkid},
		    extractor: function(data){
		    ...
		    buttons: [{
		    	text: "保  存",
		    	handler: function(e){
		   				$('#editRegionCodeFrom').form({   
						 url:'editRegionCodeAction.action',    
							     onSubmit: function(){
							       $.messager.progress(); 
							        // do some check       
							        // return false to prevent submit;    
		    ...
		    ,{
		    	text: "删  除",
		     	handler: function(e){
		     			$('#delRegionCodeFrom').form({   
						 url:'delRegionCodeAction.action',    
							     onSubmit: function(){
							       $.messager.progress(); 
							        // do some check       
							        // return false to prevent submit;    
	...
	//详细
	regionCode_list.detailFormSubmit = function(pkid){
		$('<div id="dialog_holder"></div>').dialog({
		    title: '详细RegionCode',
		    width: 800,
		    height: 500,
		    href: 'detailRegionCodePage.action',
		    modal: true,
		    method: "POST",
		    params:{pkid:pkid},
		    extractor: function(data){
