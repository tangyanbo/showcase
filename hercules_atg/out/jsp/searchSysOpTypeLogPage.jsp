...
<body class="easyui-layout">
			...
			<div class="easyui-panel" title="查询条件" style="padding:5px;height:120px;margin: 0px;" data-options="region:'north'">
				<form id="searchForm" name="searchForm" method="post" >
					<table cellpadding="5">
								...
								<td>op_type</td>
								<td>
									<input type="text" id="search_op_type" name="search_op_type" class="easyui-textbox" value="${search_op_type}"/>
								</td>
								<td>op_time</td>
								<td>
			...
			<div class="easyui-panel" style="padding:0px;margin: 0px;" data-options="region:'center'">
				<table  id="dg_sysOpTypeLog" class="easyui-datagrid" 
					...
					<thead>
						<tr>
							...
							<th data-options="field:'OP_TYPE',width:120,sortable:'true',align:'left'">op_type</th>
...
jQuery(function($){
	//定义构造查询
	sysOpTypeLog_list.buildQueryParams=function(){
	    $('#dg_sysOpTypeLog').datagrid({
			queryParams: {
						 ...
						 'search_op_type':$("input[name='search_op_type']").val(),
	...
	$('#search_btn').click(function(){
		sysOpTypeLog_list.buildQueryParams();
		$('#dg_sysOpTypeLog').datagrid('load');
	});
	...
	$('#add_btn').click(function(){
		var add_form_id ='#addSysOpTypeLogFrom';
		$('<div id="dialog_holder"></div>').dialog({
		    title: 'SYS_OP_TYPE_LOG',
		    width: 800,
		    height: 500,
		    href: 'addSysOpTypeLogPage.action',
		    modal: true,
		    method: "POST",
		    ...
		    buttons: [{
		    	text: "提  交",
		    	handler: function(e){
		    		$('#addSysOpTypeLogFrom').form({   
						 url:'addSysOpTypeLogAction.action',    
							     onSubmit: function(){
							       $.messager.progress(); 
							        // do some check       
	...
	//更新
	sysOpTypeLog_list.updateFormSubmit = function(pkid){
	    var edit_form_id ='#editSysOpTypeLogFrom';
		$('<div id="dialog_holder"></div>').dialog({
		    title: 'SysOpTypeLog',
		    width: 800,
		    height: 500,
		    href: 'editSysOpTypeLogPage.action',
		    modal: true,
		    method: "POST",
		    params:{pkid:pkid},
		    extractor: function(data){
		    ...
		    buttons: [{
		    	text: "保  存",
		    	handler: function(e){
		   				$('#editSysOpTypeLogFrom').form({   
						 url:'editSysOpTypeLogAction.action',    
							     onSubmit: function(){
							       $.messager.progress(); 
							        // do some check       
							        // return false to prevent submit;    
		    ...
		    ,{
		    	text: "删  除",
		     	handler: function(e){
		     			$('#delSysOpTypeLogFrom').form({   
						 url:'delSysOpTypeLogAction.action',    
							     onSubmit: function(){
							       $.messager.progress(); 
							        // do some check       
							        // return false to prevent submit;    
	...
	//详细
	sysOpTypeLog_list.detailFormSubmit = function(pkid){
		$('<div id="dialog_holder"></div>').dialog({
		    title: '详细SysOpTypeLog',
		    width: 800,
		    height: 500,
		    href: 'detailSysOpTypeLogPage.action',
		    modal: true,
		    method: "POST",
		    params:{pkid:pkid},
		    extractor: function(data){
